...
def create_media(
    ...
    # Only use actual EXIF GPS coordinates - no fallback
...
def create_media_batch(db: Session, user_id: str, files: list):
    ...
    Only uses actual EXIF GPS coordinates - no fallback.
    """
    media_objects = []
    for f in files:
        media_id = str(uuid.uuid4())
        ...
        print(f"🗺️  Creating media {media_id}: lat={lat}, lon={lon}, file={f.get('file_url')}")
    ...
    for media in media_objects:
        db.refresh(media)
        print(f"✅ Media {media.id} saved: lat={media.latitude}, lon={media.longitude}")
