...
def get_media_heatmap(request: Request, db: Session = Depends(get_db)):
    ...
    logger.info(f"🗺️  Heatmap request: found {len(media_list)} total media records for user {clerk_user.id}")
    ...
    for m in media_list:
        if m.latitude is not None and m.longitude is not None:
            ...
            logger.info(f"  ✅ Point: {m.id} at ({m.latitude}, {m.longitude})")
        else:
            logger.warning(f"  ⚠️  Media {m.id} missing coordinates: lat={m.latitude}, lon={m.longitude}")
    
    logger.info(f"📍 Returning {len(points)} heatmap points")
...
def create_media_batch_records(
    ...
    for idx, f in enumerate(files):
        logger.info(f"  File {idx}: lat={f.get('latitude')}, lon={f.get('longitude')}")
        f["file_type"] = "image"
    
    created_media = create_media_batch(db, clerk_user.id, files)
    
    logger.info(f"✅ Created {len(created_media)} media records")
    for media in created_media:
        logger.info(f"  Media {media.id}: lat={media.latitude}, lon={media.longitude}")
...
def get_predictions(media_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    logger.info(f"📍 Returning predictions for {media_id}: lat={media.latitude}, lon={media.longitude}")
